...
if (isset($_POST['action']) && isset($_POST['student_id'])) {
    ...
    if ($action === 'approve') {
        ...
        $query = $conn->prepare("UPDATE application_approvals 
        ...
        $query = $conn->prepare("UPDATE application_approvals 
                                 ...
                                 WHERE student_id = ?");
        $query->bind_param("sss", $status, $reason, $id);
    } else {
    ...
    if ($query->execute()) {
        ...
        if ($emailResult->num_rows > 0) {
            ...
            try {
                ...
                $mail->Password = 'knhrhuhnirfqmfgg';
                ...
                if ($status === 'Approved') {
                    ...
                    $mail->Subject = 'Dormitory Application Approved';

                    $messageContent = "
                    <p>Your dormitory application has been <b style='color:green;'>APPROVED</b>.</p>
                    <p>You may now log in to your account and sign your dormitory contract.</p>
                    ";
                } else {

                    $mail->Subject = 'Dormitory Application Rejected';

                    $messageContent = "
                    <p>Your dormitory application has been <b style='color:red;'>REJECTED</b>.</p>
                    <p><b>Reason:</b> {$reason}</p>
                    <p>If you believe this decision was made in error, you may contact the dormitory administrator.</p>
                    ";
                }
...
<tr>
<td width='60'>
<img src='cid:idormlogo' width='50'>
</td>
<td>
...
<p style='font-size:11px;color:#aaa;'>This is an automated email. Please do not reply.</p>
                ...
                if ($mail->send()) {
                    ...
                    echo json_encode([
                        'status' => 'success',
                        'message' => "Applicant has been {$status}."
                    ]);
                } else {
